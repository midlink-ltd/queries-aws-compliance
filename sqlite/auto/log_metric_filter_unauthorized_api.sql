with filter_data as (
  ...
  where
    ...
    and like_regex(filter.filter_pattern, 'errorCode\s*=\s*"\*{0,1}UnauthorizedOperation\*{0,1}"')
    and like_regex(filter.filter_pattern, 'errorCode\s*=\s*"\*{0,1}AccessDenied\*{0,1}"')
    --and like_regex(filter.filter_pattern, 'sourceIPAddress\s*!=\s*"delivery.logs.amazonaws.com"')
    --and like_regex(filter.filter_pattern, 'eventName\s*!=\s*"HeadBucket"')
