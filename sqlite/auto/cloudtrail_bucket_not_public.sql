...
-- but will be non-zero if any matches are found
  select
    ...
    t.region,
    t.account_id,
    count(acl_grant.value) filter (where json_extract(acl_grant.value,'$.Grantee.URI') like '%acs.amazonaws.com/groups/global/AllUsers') as all_user_grants,
    count(acl_grant.value) filter (where json_extract(acl_grant.value,'$.Grantee.URI') like '%acs.amazonaws.com/groups/global/AuthenticatedUsers') as auth_user_grants,
    count(s.value) filter (where json_extract(s.value,'$.Effect') = 'Allow' and  p.value = '*' ) as anon_statements
  from
    aws_cloudtrail_trail as t
  left join aws_s3_bucket as b on t.s3_bucket_name = b.name
  left join json_each(acl, '$.Grants') as acl_grant on true
  left join json_each(policy_std, '$.Statement') as s  on true
  left join json_each(s.value, '$.Principal.AWS') as p  on true
  group by
    t.s3_bucket_name,
    b.arn,
    t.region,
