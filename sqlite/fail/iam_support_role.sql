...
(
  select
    ...
    count(policy_arn.value),
    a.account_id
  from
    aws_account as a
    left join aws_iam_role as r on r.account_id = a.account_id
    left join json_each(attached_policy_arns) as policy_arn  on true
  where
    split_part(policy_arn.value, '/', 2) = 'AWSSupportAccess'
    or policy_arn.value is null
  group by
    a.account_id,
    a.partition
)
