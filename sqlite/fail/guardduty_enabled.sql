select
  ...
  case
    ...
    when status = 'ENABLED' and json_extract(data_sources,'$.S3Logs.Status') = '"ENABLED"' then 'ok'
    else 'alarm'
  end as status,
  case
    ...
    when status = 'ENABLED' and json_extract(data_sources,'$.S3Logs.Status') = '"ENABLED"' then r.title || ' enabled.'
